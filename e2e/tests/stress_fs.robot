...
Stress Volume Node FS When Replica Is Rebuilding
    ...
    FOR    ${i}    IN RANGE    ${LOOP_COUNT}
        ...
        And Stress the FS of all volume nodes

        Then Wait until replica on volume node rebuilt
        And Check data is intact
    END

Stress Volume Node FS When Volume Is Detaching and Attaching
    Given Create a volume with 5 GB and 3 replicas
    And Write data to the volume

    FOR    ${i}    IN RANGE    ${LOOP_COUNT}
        When Stress the FS of all volume nodes
...
Stress Volume Node FS When Volume Is Online Expanding
...
Stress Volume Node FS When Volume Is Offline Expanding
    @{data_checksum_list} =    Create List
    Set Test Variable    ${data_checksum_list}
    ...
    FOR    ${i}    IN RANGE    ${LOOP_COUNT}
        ...
        And Stress the FS of all worker nodes
