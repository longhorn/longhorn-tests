...
class Minio(Base):
    ...
    def set_backupstore(self, client):
        ...
        for backupstore in backupstores:
            if self.is_backupTarget_s3(backupstore):
                backupsettings = backupstore.split("$")
                self.set_backupstore_url(client, backupsettings[0])
                self.set_backupstore_credential_secret(client, backupsettings[1])
                self.set_backupstore_poll_interval(client, poll_interval)
                break
    ...
    def get_backupstore_bucket_name(self, client):
        backupstore = self.backupstore_get_backup_target(client)
    ...
    def write_backup_cfg_file(self, client, core_api, volume_name, backup_name, backup_cfg_data): # NOQA
        secret_name = self.get_secret(client)
        assert secret_name != ''
