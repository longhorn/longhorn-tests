...
class deployment_keywords:
    ...
    def create_deployment(self, name, claim_name, replicaset=1, enable_pvc_io_and_liveness_probe=False, block_volume=False, rwx_file_writing=False):
        logging(f'Creating deployment {name}')
        create_deployment(name, claim_name, replicaset=replicaset, enable_pvc_io_and_liveness_probe=enable_pvc_io_and_liveness_probe, block_volume=block_volume, rwx_file_writing=rwx_file_writing)
