...
class volume_keywords:
    ...
    def get_volume_attached_node(self, volume_name):
    ...
    def get_volume_not_attached_node(self, volume_name):
        nodes = list_nodes()
        volume_attached_node = self.get_volume_attached_node(volume_name)
        test_pod_running_node = get_test_pod_running_node()
        for node in nodes:
            if node != volume_attached_node and node != test_pod_running_node:
                return node
