...
DEP_VERSIONS_URL="https://raw.githubusercontent.com/longhorn/dep-versions"
PAGE_SIZE=100
SLEEP_SECONDS=0.2
CURL_TIMEOUT=10
...
declare -A DEP_VERSIONS_KEY_MAP=(
  [csi-attacher]=kubernetes-csi-external-attacher
  [csi-provisioner]=kubernetes-csi-external-provisioner
  [csi-resizer]=kubernetes-csi-external-resizer
  [csi-snapshotter]=kubernetes-csi-external-snapshotter
  [livenessprobe]=kubernetes-csi-livenessprobe
  [csi-node-driver-registrar]=kubernetes-csi-node-driver-registrar
  [support-bundle-kit]=rancher-support-bundle-kit
)

declare -A COMPONENT_VERSIONS COMPONENT_TAGS COMPONENT_ARTIFACT_NAMES COMPONENT_CHART_TAGS COMPONENT_DEP_TAGS

to_env_name() {
  echo "${1}_tag" | tr '[:lower:]-' '[:upper:]_'
}

get_dep_versions_branch() {
  local longhorn_version="${1}"
  local major_minor
  major_minor="$(echo "${longhorn_version}" | grep -oE '^[0-9]+\.[0-9]+')"
  echo "v${major_minor}.x"
}

fetch_dep_versions() {
  local longhorn_version="${1}"
  local branch response http_code

  branch="$(get_dep_versions_branch "${longhorn_version}")"
  echo "INFO: Fetching dep-versions from branch ${branch}" >&2

  response="$(curl --max-time "${CURL_TIMEOUT}" -sG -w "\n%{http_code}" \
    "${DEP_VERSIONS_URL}/${branch}/versions.json")"
  http_code="$(tail -n1 <<< "$response")"

  if [[ ! "$http_code" =~ ^2[0-9]{2}$ ]]; then
    echo "ERROR: Failed to fetch dep-versions for branch ${branch} (HTTP ${http_code})" >&2
    exit 1
  fi
...
parse_dep_component_versions() {
  local longhorn_version="${1}"
  local dep_versions_json dep_key component tag

  dep_versions_json="$(fetch_dep_versions "${longhorn_version}")"

  if ! jq empty <<< "$dep_versions_json" 2>/dev/null; then
    echo "ERROR: Invalid JSON from dep-versions" >&2
    exit 1
  fi

  for dep_key in "${!DEP_VERSIONS_KEY_MAP[@]}"; do
    component="${DEP_VERSIONS_KEY_MAP[${dep_key}]}"
    tag="$(jq -r ".\"${dep_key}\".tag // empty" <<< "$dep_versions_json")"

    if [[ -z "$tag" ]]; then
      echo "ERROR: Component ${dep_key} not found in dep-versions" >&2
      exit 1
    fi

    COMPONENT_DEP_TAGS["${component}"]="${tag#v}"
...
parse_component_versions() {
  ...
  parse_dep_component_versions "$1"
...
print_tag_comparison() {
  local component chart_tag appco_tag dep_tag env_name
  local all_current=true

  echo ""
  echo "=== AppCo Revision Tag Comparison ==="
  printf "%-45s %-20s %-20s %-20s %-10s\n" \
    "COMPONENT" "DEP-VERSIONS TAG" "CHART TAG" "APPCO LATEST TAG" "STATUS"
  printf '%*s\n' 115 '' | tr ' ' '-'

  for component in "${DEPENDENCY_COMPONENTS[@]}" "${LONGHORN_COMPONENTS[@]}"; do
    dep_tag="${COMPONENT_DEP_TAGS[$component]:-N/A}"
    ...
    if [[ "$chart_tag" == "$appco_tag" ]]; then
      printf "%-45s %-20s %-20s %-20s %-10s\n" \
        "$component" "$dep_tag" "$chart_tag" "$appco_tag" "UP-TO-DATE"
    else
      printf "%-45s %-20s %-20s %-20s %-10s\n" \
        "$component" "$dep_tag" "$chart_tag" "$appco_tag" "OUTDATED"
      all_current=false
    fi
  done
...
fetch_appco_revision_tags() {
  ...
  for component in "${DEPENDENCY_COMPONENTS[@]}"; do
    if [[ -z "${COMPONENT_VERSIONS[$component]:-}" ]]; then
      echo "ERROR: Cannot find image version for dependency component ${component} from dep-versions" >&2
      exit 1
    fi
