...
if [ "$#" -ne 3 ]; then
    ...
    echo "  <new_version>       The new version of Longhorn images to use (e.g., v1.9.1-dev-20250629, master-head)."
    ...
    echo "    $0 v1.7.x-head v1.7.4-dev-20250629 v1.7.x"
    echo ""
    echo "    # For downloading longhorn.yaml from the v1.8.x branch"
    echo "    $0 v1.8.x-head v1.8.3-dev-20250629 v1.8.x"
    echo ""
    echo "    # For downloading longhorn.yaml from the v1.9.x branch"
    echo "    $0 v1.9.x-head v1.9.1-dev-20250629 v1.9.x"
    echo ""
    echo "    # For downloading longhorn.yaml from the master branch"
    echo "    $0 master-head v1.10.0-dev-20250629 master"
    echo ""
    exit 1
fi
