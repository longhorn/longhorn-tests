...
def test_fast_replica_rebuild_replica_deletion(client, volume_name, settings_reset):  # NOQA
    """
    1. Update data-integrity settings
    2. Create a volume and its snapshots. The snapshot should be hashed.
    3. Delete local replica and wait for the finish of the replica rebuilding.
    4. Check local replica's snapshot files are existing and their.
...
def test_fast_replica_rebuild_replica_corruption_in_global_enabled_mode(client, volume_name, settings_reset):  # NOQA
    update_settings(client, SNAPSHOT_DATA_INTEGRITY_ENABLED, "true", "true")
    detect_and_repair_corrupted_replica(client, volume_name,
                                        SNAPSHOT_DATA_INTEGRITY_ENABLED)


def test_fast_replica_rebuild_replica_corruption_in_global_fast_check_mode(client, volume_name, settings_reset):  # NOQA
    update_settings(client, SNAPSHOT_DATA_INTEGRITY_FAST_CHECK, "true", "true")
    detect_and_repair_corrupted_replica(client, volume_name,
                                        SNAPSHOT_DATA_INTEGRITY_FAST_CHECK)
