...
@pytest.mark.backing_image  # NOQA
def test_backing_image_eviction(client): # NOQA
    """
    Test Node Eviction
    ...
    Test Disk Eviction
    - Disable the eviction on the previous node
    - Evict the disk where the copy is currently on
    - The BackingImage will be evicted to another disk
    """
    ...
    backing_image = client.by_id_backing_image(BACKING_IMAGE_NAME)
    disk_id_1 = next(iter(backing_image.diskFileStatusMap))

    node_1 = get_node_by_disk_id(client, disk_id_1)
    ...
    check_backing_image_single_copy_node_eviction(
        client, BACKING_IMAGE_NAME, node_1)

    backing_image = client.by_id_backing_image(BACKING_IMAGE_NAME)
    disk_id_2 = next(iter(backing_image.diskFileStatusMap))
    node_2 = get_node_by_disk_id(client, disk_id_2)

    node_1 = set_node_scheduling_eviction(
        client, node_1, allowScheduling=True, evictionRequested=False)
    node_2 = set_node_scheduling_eviction(
        client, node_2, allowScheduling=False, evictionRequested=True)

    check_backing_image_disk_map_status(client, BACKING_IMAGE_NAME, 1, "ready")
    check_backing_image_single_copy_disk_eviction(
        client, BACKING_IMAGE_NAME, disk_id_2)
