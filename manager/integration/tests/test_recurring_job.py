...
from common import create_and_wait_deployment, DATA_SIZE_IN_MB_2
...
from common import wait_for_volume_attached, RETRY_BACKUP_COUNTS
...
def wait_for_recurring_backup_to_start(client, core_api, volume_name, n, progress=50):  # NOQA
...
def test_recurring_jobs_when_volume_detached_unexpectedly(backupstore_s3, client, core_api, apps_api, pvc, make_deployment_with_pvc):  # NOQA
    """
    ...
    wait_for_recurring_backup_to_start(client, core_api, vol_name, 1)

    crash_engine_process_with_sigkill(client, core_api, vol_name)
    time.sleep(30)
    wait_for_volume_attached(client, vol_name)

    snapshot_name = wait_for_recurring_backup_to_start(client, core_api,
                                                       vol_name, 1)
    ...
    assert read_volume_data(core_api, pod_names[0], 'default') is not None
